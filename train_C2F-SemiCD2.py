    ...
    with tqdm(total=len(train_loader), desc=f'Eps {epoch}/{num_epoches}', unit='img') as pbar:
        for i, (A, B, mask,with_label) in enumerate(train_loader): #with_label是？
            ...
            if use_ema is True:
                ...
                loss=loss+0.2*loss_semi  #为啥是0.2？ 全监督损失+半监督损失，可以调
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_ratio', type=float, default=0.1, help='Proportion of the labeled images')  # 修改这里！！！
    parser.add_argument('--clip', type=float, default=0.5, help='gradient clipping margin')
    parser.add_argument('--decay_rate', type=float, default=0.1, help='decay rate of learning rate')
    parser.add_argument('--decay_epoch', type=int, default=50, help='every n epochs decay learning rate')
    parser.add_argument('--gpu_id', type=str, default='3', help='train use gpu')  # 修改这里！！！
    parser.add_argument('--data_name', type=str, default='LEVIRsup-WHUunsup',  # 修改这里！！！
    ...
    train_loader = data_loader.get_semiloader(opt.train_root, opt.batchsize, opt.trainsize, opt.train_ratio,
                                              num_workers=8, shuffle=True, pin_memory=False)
    val_loader = data_loader.get_test_loader(opt.val_root, opt.batchsize, opt.trainsize, num_workers=6, shuffle=False,
                                             pin_memory=False)
    ...
    student_dir = save_path +  '_train1_' + '_best_student_iou.pth'
    teacher_dir =save_path +  '_train1_' + '_best_teacher_iou.pth'
    checkpoint_stud = torch.load(student_dir)
    model.load_state_dict(checkpoint_stud['best_student_net '])
    optimizer.load_state_dict(checkpoint_stud['optimizer '])
    ema_model.load_state_dict(torch.load(teacher_dir))
    print('student_dir',student_dir)
    print('teacher_dir',teacher_dir)

    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=15, T_mult=2)
    current_epoch = checkpoint_stud[' epoch'] + 1
    ...
    for epoch in range(current_epoch, opt.epoch):
    # for epoch in range(1, opt.epoch):
        ...
        Eva_val.reset()
        Eva_val2.reset()
        # train(train_loader, val_loader, Eva_train,Eva_train2, Eva_val,Eva_val2, data_name, save_path, model,
        #       ema_model, criterion,semicriterion, optimizer,use_ema, opt.epoch)
        train1(train_loader, val_loader, Eva_train, Eva_train2, Eva_val, Eva_val2, data_name,
               save_path, model,ema_model, criterion, semicriterion, optimizer, use_ema, opt.epoch)
