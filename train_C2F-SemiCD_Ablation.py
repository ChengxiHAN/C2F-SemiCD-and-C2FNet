    ...
    with tqdm(total=len(train_loader), desc=f'Eps {epoch}/{num_epoches}', unit='img') as pbar:
        for i, (A, B, mask, with_label) in enumerate(train_loader):  # with_label是？
            ...
            if use_ema is False:
                ...
                preds = net(A, B)
                if with_label.any():
                    loss = criterion(preds[0][with_label], Y[with_label]) + criterion(preds[1][with_label],
                                                                                      Y[with_label])
                else:
                    loss = 0
            ...
            if use_ema is True:
                ...
                with torch.no_grad():
                    ...
                    pseudo_attn, pseudo_preds = torch.sigmoid(pseudo_attn).detach(), torch.sigmoid(
                        pseudo_preds).detach()
                loss_semi = semicriterion(preds[0][~with_label], pseudo_attn) + semicriterion(preds[1][~with_label],
                                                                                              pseudo_preds)  # 测试这里的效果，如果有用则方便讲故事
...
if __name__ == '__main__':
    ...
    parser.add_argument('--epoch', type=int, default=80, help='epoch number')  # 修改这里！！！
    ...
    parser.add_argument('--save_path', type=str, default='./output/LEVIR-10-without_GCM_A_B_C/')  # 半监督的路径
    ...
    for epoch in range(1, opt.epoch):
        ...
        #        ema_model, criterion, semicriterion, optimizer, use_ema, opt.epoch)
        Ablation_train1(train_loader, val_loader, Eva_train, Eva_train2, Eva_val, Eva_val2, data_name, save_path, model,
               ema_model, criterion, semicriterion, optimizer, use_ema, opt.epoch)
